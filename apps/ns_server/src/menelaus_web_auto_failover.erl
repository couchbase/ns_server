...
%% The upgrades are complicated by using, first in phoenix, the
...
%% an upgrade from a pre-phoenix release the profile must be checked to
...
config_upgrade_to_phoenix_test() ->
    ...
    [{set, auto_failover_cfg, DefaultUpgradedCfg}] =
        config_upgrade_to_phoenix([BaseConfig]),
    ...
    [{set, auto_failover_cfg, IgnoreExistingDiskNonRespCfg}] =
        config_upgrade_to_phoenix([[{?ROOT_CONFIG_KEY, ExistingCfg}]]),
