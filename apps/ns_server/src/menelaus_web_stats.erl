...
%% Stats generated by prometheus during scrapes do not have an explict
%% 'name' label but do have the internal '__name__' label.
maybe_change_label_name(Props) ->
    Labels = proplists:get_value(metric, Props),
    MetricName = extract_metric_name(Labels),
    case is_prometheus_scrape_stat(MetricName) of
        false ->
            Props;
        true ->
            %% Change {label, name} to {label, __name__} for prometheus
            %% generated "scrape" stats.
            [{Labels1}] = Labels,
            Labels2 = lists:map(fun ({<<"label">>, <<"name">>}) ->
                                        {<<"label">>, <<"__name__">>};
                                    (Other) ->
                                        Other
                                end, Labels1),
            lists:keyreplace(metric, 1, Props, {metric, [{Labels2}]})
        end.

send_metrics_request(Props0, PermFilters) ->
    Props = maybe_change_label_name(Props0),
...
%% Each time prometheus scrapes an instance it stores the following stats.
is_prometheus_scrape_stat(<<"up">>) -> true;
is_prometheus_scrape_stat(<<"scrape_duration_seconds">>) -> true;
is_prometheus_scrape_stat(<<"scrape_samples_post_metric_relabeling">>) -> true;
is_prometheus_scrape_stat(<<"scrape_samples_scraped">>) -> true;
is_prometheus_scrape_stat(<<"scrape_series_added">>) -> true;
is_prometheus_scrape_stat(_) -> false.
