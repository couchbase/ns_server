...
%% needed for REST APIs existing prior to phoenix but whose permissions
%% changed in phoenix.
when_phoenix(NewPermission, OldPermission) ->
        ...
        Method when Method =:= 'GET'; Method =:= 'HEAD' ->
            case PathTokens of
                ...
                ["settings", "saslauthdAuth"] ->
                    {when_phoenix({[admin, security], read},
                ...
                ["settings", "rbac", "lookupLDAPUser", Name] ->
                    {when_phoenix({[admin, security_info], read},
                                   {[admin, security, external], read}),
        ...
        'POST' ->
            case PathTokens of
                ...
                ["settings", "replications", _XID] ->
                    {no_check, fun goxdcr_rest:proxy/1};
                ["settings", "saslauthdAuth"] ->
                    {when_phoenix({[admin, security], write},
                ...
                ["settings", "invalidateLDAPCache"] ->
                    {when_phoenix({[admin, security_info], write},
                                   {[admin, security, external], write}),
        ...
        'DELETE' ->
            case PathTokens of
                ...
                ["settings", "rbac", "users", UserId] ->
                    {when_phoenix({[admin, users], write},
                                   {[admin, security], write}),
