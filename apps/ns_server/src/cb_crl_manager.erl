...
-record(crl_reload_status, {
    ...
    etag :: binary() | undefined, %% etag from the most recent successull URL
                                  %% fetch
    result :: loaded | failed | not_attempted,
    %% When the last attempt happened; undefined when there was nothing to
    %% attempt, in which case 'errors' says why.
...
-record(state, {
    ...
    %% Outcome of the most recent load of the generated CRL, keyed by base name.
    %% Kept apart from 'generated' because build_file_versions/1 (and hence the
    %% CRL version) is derived from the checksum maps: a timestamp in there
    %% would bump the version on every reload.
    generated_file_state :: #{BaseName :: string() => reload_status()},
...
%% Reported against the generated CRL when the cluster has none.  Says the same
%% thing as menelaus_web_crl:reply_no_ootb_crl/0 and ns_server_cert's warning,
%% shorter: this one shares a line with the rest of a file's status.
-define(NO_OOTB_CRL_ERROR,
        <<"The out-of-the-box CRL is missing. Check whether the cluster's own "
          "CA needs to be regenerated to issue one.">>).
    ...
    case ns_server_cert:get_ootb_crl() of
        undefined ->
            maps:foreach(fun (Name, _) -> remove_from_cache(generated, Name) end,
                         Generated),
            %% Keep reporting the slot, as 'not_attempted' with the reason: a
            %% cluster whose CA cannot issue CRLs legitimately has none
            %% (MB-73096), and an omitted entry makes that indistinguishable
            %% from a version that does not generate one at all.
            Status = #crl_reload_status{result = not_attempted,
                                        time   = undefined,
                                        errors = [?NO_OOTB_CRL_ERROR]},
            State#state{generated = #{},
                        generated_file_state =
                            #{?GENERATED_CRL_NAME => Status}};
        CrlPem when is_binary(CrlPem) ->
            Name = ?GENERATED_CRL_NAME,
            Checksum = file_checksum(CrlPem),
    ...
    maybe
        ...
        {error, Reason} ->
            ...
            ?log_error("Failed to load the OOTB CRL:~n~s",
                       [lists:join(<<"\n">>, ErrorStrings)]),
            %% 'generated' is left alone: the copy we already hold (if any) is
            %% still the one on disk and in the cache.  Its checksum no longer
            %% matches chronicle, so the next reconcile retries.
            BadStatus = #crl_reload_status{result = failed, time = TS,
                                           errors = ErrorStrings},
            State#state{generated_file_state = maps:put(Name, BadStatus, GenFS)}
    end.
...
build_status_map(TrustedDerCAs,
                 #state{file_state       = FS,
                        ...
                        uploaded         = Uploaded,
                        generated        = Generated,
                        generated_file_state = GeneratedFS,
    ...
    %% Generated (OOTB) CRL entries — generated_file_state is the authoritative
    %% list, so a CRL that failed to load, or that the cluster does not have at
    %% all, is reported rather than omitted.
    GeneratedList =
        maps:fold(
...
%% Every source that keeps a #crl_reload_status{} reports its time through this
%% one function - including the generated CRL, which used to hard-code
%% 'undefined' and so showed a null lastReload.time.
last_reload_map_test() ->
    TS = {{2026, 1, 1}, {0, 0, 0}},
    ?assertEqual(#{result => not_attempted, time => undefined, errors => []},
                 last_reload_map(undefined)),
    ?assertEqual(#{result => loaded, time => TS, errors => []},
                 last_reload_map(#crl_reload_status{result = loaded,
                                                    time = TS, errors = []})),
    ?assertEqual(#{result => failed, time => TS, errors => [<<"err">>]},
                 last_reload_map(#crl_reload_status{result = failed, time = TS,
                                                    errors = [<<"err">>]})),
    %% Nothing to load is not a failure, but it still says why.
    ?assertEqual(#{result => not_attempted, time => undefined,
                   errors => [?NO_OOTB_CRL_ERROR]},
