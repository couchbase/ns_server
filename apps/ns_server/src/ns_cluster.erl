...
engage_cluster_apply_certs(NodeKVList) ->
    CanRewriteNodeCerts =
        ns_server_cert:this_node_uses_self_generated_node_certs(),
        ...
        Cert when Cert =/= undefined, CanRewriteNodeCerts andalso
            ...
            ClientKey = UnwrapValue(
                          proplists:get_value(<<"autogeneratedClientKey">>,
                                              NodeKVList)),
            %% Optional: undefined when joining a pre-CRL-support cluster, in
            %% which case the OOTB CRL arrives later via chronicle replication.
            CACRL = proplists:get_value(<<"autogeneratedCRL">>,
                                        NodeKVList, undefined),
...
do_add_node_with_connectivity(Scheme, RemoteAddr, RestPort, HiddenAuth,
    ...
    GeneratedCerts =
        ns_server_cert:this_node_uses_self_generated_node_certs(Config),
...
engage_cluster_certs_props(Config, RemoteAddr) ->
    ...
    NodeCerts =
        case ns_server_cert:this_node_uses_self_generated_node_certs(Config) of
            true ->
    ...
    misc:update_proplist(NodeCerts, ClientCerts) ++ build_ca_certificates().

%% Ship the cluster's OOTB CRL so a joining node can validate the cluster's
%% generated certs during the join handshake, before chronicle replication
%% delivers the ootb_crl key.  Omitted when the cluster has no OOTB CRL, e.g. it
%% has not yet been upgraded to a CRL-supporting version.
build_ca_crl() ->
    case ns_server_cert:get_ootb_crl() of
        undefined -> [];
        CRL       -> [{<<"autogeneratedCRL">>, CRL}]
    end.
...
build_ca_certificates() ->
    [{<<"trustedCAs">>,
      lists:map(
        fun (Cert) ->
                %% id and pem are used for logging and validation respectively
                RetainedKeys = [id, pem],
                {lists:filter(
                   fun ({Key, _}) -> lists:member(Key, RetainedKeys) end,
                   Cert)}
        end,
...
do_engage_cluster_validate_cert(NodeKVList, Services) ->
    maybe
        TrustedCAs = proplists:get_value(<<"trustedCAs">>, NodeKVList),
        ok ?= validate_local_certificate_chain(node_cert, TrustedCAs),
        ok ?= validate_local_certificate_chain(client_cert, TrustedCAs),
        ok ?= validate_local_node_certificate_san(),
...
%% We need to validate our local certificate chains (node and client) against
%% the cluster's trusted CAs, because the connectivity checks are all performed
%% via Erlang's SSL implementation, which can sometimes provide the local
%% node's trustedCAs as part of the node/client chain.
%% Those certs may include an intermediate certificate in the chain, between
%% this node's node/client cert, and the cluster's RootCA, which allows the
%% cluster to validate the node/client cert prior to it joining the cluster.
%% Once the node has joined the cluster however, the cluster's trustedCAs will
%% be used instead, which could mean that the intermediate certificate is no
%% longer included in the chain.
%% By validating the node/client certs against the cluster's trustedCAs, we
...
validate_local_certificate_chain(CertType, TrustedCAs) ->
    maybe
        CertInfo = ns_server_cert:get_cert_info(CertType, node()),
        Pem = proplists:get_value(pem, CertInfo),
        %% Only validate if node-to-be-added is not using self generated certs
        ok ?=
            case ns_server_cert:this_node_uses_self_generated_certs(CertInfo) of
                true -> {error, self_generated_certs};
                false -> ok
            end,
        ParsedCAs = lists:map(
                      fun ({Cert}) ->
                              %% id is used for logging purposes, pem is for
                              %% actually validating the cert chain
                              [{id, proplists:get_value(<<"id">>, Cert)},
                               {pem, proplists:get_value(<<"pem">>, Cert)}]
                      end,
                      TrustedCAs),
        %% Return when no errors, otherwise construct the error message
        {ok, _, _} ?= ns_server_cert:decode_and_validate_chain(ParsedCAs, Pem),
        ok
    else
        {error, self_generated_certs} ->
            ok;
        {error, Error} ->
            CertTypeStr = case CertType of
                              node_cert -> "node";
                              client_cert -> "client"
                          end,
            CertErr = ns_error_messages:reload_node_certificate_error(Error),
            Message = iolist_to_binary(
                        ["Error validating new node's ", CertTypeStr,
                         " certificate chain. ", CertErr]),
            {error, CertType, Message}
    end.
