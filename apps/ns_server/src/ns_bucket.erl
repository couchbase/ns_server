...
all_keys_by_uuid(BucketUUIDs, SubKeys, Txn) ->
    case cluster_compat_mode:is_cluster_phoenix() of
        true ->
            lists:flatmap(
                fun (BucketUUID) ->
                    all_bucket_keys_by_uuid_phoenix(BucketUUID, SubKeys, Txn)
                end, BucketUUIDs);
        false ->
            ...
            lists:flatmap(
                fun (BucketUUID) ->
                        all_bucket_keys_by_uuid_pre_phoenix(BucketUUID,
                                                             SubKeys,
                                                             UUIDSnapshot)
                end, BucketUUIDs)
    end.
...
all_bucket_keys_by_uuid_pre_phoenix(BucketUUID, SubKeys, Snapshot) ->
...
%% Once phoenix is the oldest supported release all vestiges of 'memcached'
%% can be removed. Until then it's possible for a phoenix node to see a
...
get_default_num_vbuckets(magma) ->
    Default = case cluster_compat_mode:is_cluster_phoenix() of
                  true -> ?DEFAULT_VBUCKETS_MAGMA;
                  false -> ?DEFAULT_VBUCKETS_MAGMA_PRE_PHOENIX
              end,
...
chronicle_add_uuid2bucket_mapping_upgrade_to_phoenix(BucketName, Txn) ->
...
chronicle_upgrade_bucket_props_to_phoenix(BucketName, ChronicleTxn) ->
    PropsKey = sub_key(BucketName, props),
...
chronicle_upgrade_to_phoenix(ChronicleTxn) ->
    ...
    chronicle_upgrade_bucket(
        fun (Name, Txn) ->
            functools:chain(
              Txn,
              [chronicle_upgrade_bucket_props_to_phoenix(Name, _),
               chronicle_add_uuid2bucket_mapping_upgrade_to_phoenix(Name, _)])
