...
is_compatible_past_map(OptionsPast0, OptionsNow0) ->
    %% vbmaps stored in vbucket_map_history include the opts used to generate a
    %% given vbmap. In 7.1.1 we introduced an option
    %% 'use_vbmap_greedy_optimization' which was to be used to disable the
    %% greedy vbmap algorithm.

    %% When we do a delta recovery of a node upgraded to 7.1.1 and try to
    %% compare the existing vbmaps opts, none of them match vbmap opts for
    %% vbmap generated by pre-7.1.1 versions. Hence ignore
    %% 'use_vbmap_greedy_optimization' opt when comparing the maps below.
    IgnoreOpts = [tags, use_vbmap_greedy_optimization, maps_history,
                  terse_output, uploaders],
    {OptionsPast, OptionsNow} =
        lists:foldl(fun (Opt, {OptsPast, OptsNew}) ->
                            {lists:keydelete(Opt, 1, OptsPast),
                             lists:keydelete(Opt, 1, OptsNew)}
                    end, {OptionsPast0, OptionsNow0},
                    IgnoreOpts),
...
find_matching_past_maps(Nodes, Map, MapOptions, History, Options) ->
    Options1 = lists:sort(MapOptions),
...
invoke_vbmap(CurrentMap, Nodes, NumVBuckets, NumSlaves, NumReplicas, Tags,
    ...
    UploadersFile = path_config:tempfile("prev-uploaders", ".json"),

    try
        {ok, Map} = do_invoke_vbmap(VbmapPath, DiagPath, PrevMapFile,
                                    UploadersFile, CurrentMap, Nodes,
    ...
    after
        ...
        file:delete(UploadersFile)
    end.

do_invoke_vbmap(VbmapPath, DiagPath, PrevMapFile, UploadersFile,
                CurrentMap, Nodes, NumVBuckets, NumSlaves, NumReplicas, Tags,
                UseGreedy, Uploaders, Verbose) ->
    misc:executing_on_new_process(
      fun () ->
              do_invoke_vbmap_body(VbmapPath, DiagPath, PrevMapFile,
                                   UploadersFile, CurrentMap,
                                   Nodes, NumVBuckets, NumSlaves, NumReplicas,
                                   Tags, UseGreedy, Uploaders, Verbose)
      end).

do_invoke_vbmap_body(VbmapPath, DiagPath, PrevMapFile, UploadersFile,
                     CurrentMap, Nodes, NumVBuckets, NumSlaves, NumReplicas,
                     Tags, UseGreedy, Uploaders, Verbose) ->
    ...
    IdUploaders =
        case Uploaders of
            undefined ->
                undefined;
            _ ->
                [node_to_id(Node, NodeIdMap) || {Node, _} <- Uploaders]
        end,

    Args = vbmap_tags_args(NodeIdMap, Tags) ++ Args0 ++
        maybe_write_json_to_file(IdVbMap, PrevMapFile, vbmap,
                                 "--current-map") ++
        maybe_write_json_to_file(IdUploaders, UploadersFile, uploaders,
                                 "--current-uploaders"),
...
make_vbmap_with_node_ids(NodeIdMap, CurrentMap) ->
    [[node_to_id(Node, NodeIdMap) || Node <- Chain] || Chain <- CurrentMap].
...
write_json_to_file(Json, Filename) ->
    try
        file:write_file(Filename, ejson:encode(Json))
    catch T1:E1:S1 ->
            {error, {T1, E1, S1}}
    end.

maybe_write_json_to_file(undefined, _Filename, _Name, _Arg) ->
    [];
maybe_write_json_to_file(Json, Filename, Name, Arg) ->
    case write_json_to_file(Json, Filename) of
        ok ->
            ?log_debug("Wrote ~p to ~p", [Name, Filename]),
            [Arg, Filename];
        Err ->
            ?log_error("Couldn't write ~p to file: ~p, reason: ~p",
                       [Name, Filename, Err]),
            exit({cannot_write_file, Filename, Err})
    end.
