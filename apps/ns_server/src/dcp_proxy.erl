...
suppress_logging(<<?REQ_MAGIC:8, ?DCP_SYSTEM_EVENT:8, _Rest/binary>>) ->
    true;
suppress_logging(<<?RES_MAGIC:8, ?DCP_CACHED_VALUE:8, _KeyLen:16, _ExtLen:8,
                   _DataType:8, ?SUCCESS:16, _Rest/binary>>) ->
    true;
suppress_logging(<<?REQ_MAGIC:8, ?DCP_CACHED_VALUE:8, _Rest/binary>>) ->
    true;
suppress_logging(<<?RES_MAGIC:8, ?DCP_CACHED_KEY_META:8, _KeyLen:16, _ExtLen:8,
                   _DataType:8, ?SUCCESS:16, _Rest/binary>>) ->
    true;
suppress_logging(<<?REQ_MAGIC:8, ?DCP_CACHED_KEY_META:8, _Rest/binary>>) ->
    true;
suppress_logging(<<?RES_MAGIC:8, ?DCP_CACHE_TRANSFER_END:8, _KeyLen:16,
...
connect_inner(Cfg, Node, RepFeatures) ->
    ...
    {Protocol, Opts, Port} =
        case proplists:get_bool(ssl, RepFeatures) andalso Node =/= node() of
            true ->
                {ssl, SOpts ++ ns_ssl_services_setup:tls_client_opts(Cfg, []),
                 SslPort};
            false ->
                {tcp, SOpts, TcpPort}
        end,
