...
-type secret_props() :: #{key_resource_id := string(),
...
persist(Props, _ExtraAD, _) ->
    ensure_gcp_kek_on_disk(Props, false).
...
test_props(#{stored_ids := [StoredId | _]} = Props, _ExtraAD, _) ->
    ...
    case ensure_gcp_kek_on_disk(PropsWithoutHistKeys, true) of
        ok -> ok;
...
ensure_gcp_kek_on_disk(#{stored_ids := StoredIds} = Props, TestOnly) ->
    Params = maps:with([key_resource_id, credentials_file, req_timeout_ms],
                        Props),
    Res = lists:map(
            fun (#{id := Id, creation_time := CreationTime}) ->
                {Id, encryption_service:store_gcp_key(Id, Params, CreationTime,
                                                      TestOnly)}
            end, StoredIds),
