...
start_link(Vb, Id, Parent, Options) ->
    {ok, Pid} = gen_server:start_link(?MODULE, {Vb, Id, Parent, Options}, []),
    {ok, Pid}.
...
handle_call({flush_docs_pipeline, DocsList}, _From,
                    ...
                    _NumDocsSent =
                        lists:foldr(
                          fun (#doc{id = Key, rev = Rev, deleted = Deleted,
                                  ...
                                  ok = mc_cmd_pipeline(OpCode, Sock, {McHeader, McBody}, undefined),
                                  Acc + 1
                          end,
                          0,
                          DocsList)
    ...
    {Flushed, Enoent, Eexist, NotMyVb, Einval, Timeout} =
        lists:foldr(fun(#doc{id = _Key} = _Doc,
                        {FlushedAcc, EnoentAcc, EexistsAcc, NotMyVbAcc, EinvalAcc, Timeout}) ->
                            case get_flush_response_pipeline(Sock, VBucket) of
                                {ok, _, _} ->
                                    {FlushedAcc + 1, EnoentAcc, EexistsAcc, NotMyVbAcc, EinvalAcc, Timeout};
                                {memcached_error, key_enoent, _} ->
                                    {FlushedAcc, EnoentAcc + 1, EexistsAcc, NotMyVbAcc, EinvalAcc, Timeout};
                                {memcached_error, key_eexists, _} ->
                                    {FlushedAcc, EnoentAcc, EexistsAcc + 1, NotMyVbAcc, EinvalAcc, Timeout};
                                {memcached_error, not_my_vbucket, _} ->
                                    {FlushedAcc, EnoentAcc, EexistsAcc, NotMyVbAcc + 1, EinvalAcc, Timeout};
                                {memcached_error, einval, _} ->
                                    {FlushedAcc, EnoentAcc, EexistsAcc, NotMyVbAcc, EinvalAcc + 1, Timeout};
                                {memcached_error, timeout, _} ->
                                    {FlushedAcc, EnoentAcc, EexistsAcc, NotMyVbAcc, EinvalAcc, Timeout + 1}
                            end
                    end,
...
-spec flush_single_doc(integer(), inet:socket(), integer(), #doc{}, integer()) -> ok |
                                                                       {error, {term(), term()}}.
...
flush_single_doc(Id, Socket, VBucket,
    ...
    ConflictRes = case ?XDCR_LOCAL_CONFLICT_RESOLUTION of
             false ->
...
receive_remote_meta_pipeline(Sock, VBucket) ->
    ...
    {ok, RecvHeader, RecvEntry} = Response,
    Raw = case Response of
              ...
              Response ->
                  ...
                  mc_client_binary:process_error_response({ok, RecvHeader, RecvEntry, undefined})
          end,
    ...
    Reply = case Raw of
                ...
                OtherErr ->
                    ErrorMsg = ?format_msg("other error from remote: ~p (vb: ~p)",
                                           [OtherErr, VBucket]),
...
mc_cmd_pipeline(Opcode, Sock, {Header, Entry}, _Timeout) ->
