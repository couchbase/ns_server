...
%% Copyright Couchbase, Inc 2013-2018. All Rights Reserved.
%%
...
-define(TIMER_TAB, timer_tab2).
-define(INTERVAL_TAB, timer_interval_tab2).

-define(SERVER, timer2_server).
...
ensure_started() ->
	...
	undefined ->
	    C = {?SERVER, {?MODULE, start_link, []}, permanent, 1000,
		 worker, [?MODULE]},
...
req(Req, Arg) ->
    SysTime = system_time_millis(),
    ensure_started(),
    gen_server:call(?SERVER, {Req, Arg, SysTime}, infinity).
...
handle_call({apply_after, {Time, Op}, Started}, _From, _Ts)
  when is_integer(Time), Time >= 0 ->
    BRef = {Started + Time, make_ref()},
    Timer = {BRef, timeout, Op},
    ets:insert(?TIMER_TAB, Timer),
    Timeout = timer_timeout(system_time_millis()),
...
next_timeout() ->
	...
	'$end_of_table' ->
	    infinity;
	{Time, _} ->
	    erlang:min(positive(Time - system_time_millis()), ?MAX_TIMEOUT)
    end.
