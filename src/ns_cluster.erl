...
engage_cluster_apply_certs(NodeKVList) ->
    ...
    case proplists:get_value(<<"autogeneratedCA">>, NodeKVList) of
        undefined ->
            ...
            %% there are some cases when it's still possible that user hasn't
        ...
        Cert when Cert =/= undefined, CanRewriteNodeCerts orelse
                                      CanRewriteClientCerts ->
...
post_json(Target, Auth, Options, Stuff) ->
    ...
    Post = list_to_tuple(Target ++
                         ["application/json", ejson:encode(Stuff)]),
...
engage_cluster_certs_props(Config, RemoteAddr) ->
    ...
    %% Wrap sensitive values in {otpCookie, _} to make sure it is sanitized
    %% by pre-7.0 nodes
    MaybeWrapValue =
        fun (Value, Vsn) ->
            case cluster_compat_mode:is_enabled(Vsn) of
                true -> Value;
                false -> {[{otpCookie, Value}]}
            end
        end,
    ...
    NodeCerts =
        case ns_server_cert:this_node_uses_self_generated_certs(Config) of
            true ->
                ...
                %% new node with certs)
                Key = MaybeWrapValue(NewNodeKey, ?VERSION_70),
                [{<<"autogeneratedCA">>, CA},
            ...
            false ->
                ...
                %% sent in engageCluster. Can be removed when support of 7.0 is
                %% dropped.
    ...
    ClientCerts =
        case ns_server_cert:this_node_uses_self_generated_client_certs(Config) of
            true ->
                ...
                ClientKey = MaybeWrapValue(NewClientKey, ?VERSION_71),
                [{<<"autogeneratedCA">>, CA2},
                 {<<"autogeneratedClientCert">>, NewClientCert}] ++
                [{<<"autogeneratedClientKey">>, ClientKey}];
            false -> []
        end,
