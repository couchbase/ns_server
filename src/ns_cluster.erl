    ...
    case proplists:get_value(<<"clusterCA">>, NodeKVList) of
        undefined ->
            ...
            case proplists:get_value(<<"autogeneratedCA">>, NodeKVList) of
                Cert when Cert =/= undefined, CanRewriteCerts ->
                ...
                _ ->
                    ?log_info("Not using autogenerated certs because "
                              "they were not provided or this node uses custom "
                              "certs"),
                    ok
            end,
            engage_cluster_apply_net_config(NodeKVList);
...
do_add_node_with_connectivity(Scheme, RemoteAddr, RestPort, Auth, GroupUUID,
    ...
    MustUseHttps =
        ...
        proplists:is_defined(<<"autogeneratedKey">>, Props1) or
        %% We added https in 6.5 (we also made it a default)
        %% So if the cluster is 6.5 all, the nodes can use https
        %% for node addition.
        (cluster_compat_mode:is_cluster_65() and IsEnterprise),
