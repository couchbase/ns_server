...
%% Convert scram-sha auth info generated by pre-trinity code to correct
%% scram-sha auth info.
%% See MB-52422 for details.
fix_pre_trinity_auth_info(Props) ->
    lists:map(
...
pre_trinity_auth_info_key(sha) -> <<"sha1">>.
...
find_auth_info(Sha, Name) ->
    case find_auth(Name) of
        {false, _} ->
            undefined;
        {AuthInfo, Domain} ->
            MigratedAuthInfo = fix_pre_trinity_auth_info(AuthInfo),
