...
1. **Ensure prerequisites are met** — the credential store requires all nodes at 8.5+, Enterprise edition, config encryption enabled, **and** n2n encryption enabled on all nodes before any credential operation succeeds, including consume.
