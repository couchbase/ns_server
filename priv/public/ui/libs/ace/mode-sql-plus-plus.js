...
(function() {
  ...
  var keywords = (
      "adapter|all|alter|analytics|and|any|apply|args|as|asc|at|autogenerated|between|btree|by|case|closed|collection|compact|compaction|connect|connected|correlate|create|cube|current|dataset|dataverse|declare|definition|delete|desc|disable|disconnect|distinct|div|drop|element|else|enable|end|enforced|every|except|exclude|exists|explain|external|feed|filter|first|flatten|following|for|from|full|fulltext|function|group|grouping|groups|having|hints|if|ignore|in|index|ingestion|inner|insert|internal|intersect|into|is|join|key|keyword|known|last|left|let|letting|like|limit|link|load|missing|mod|ngram|no|nodegroup|not|null|nulls|offset|on|open|or|order|others|outer|output|over|partition|path|policy|preceding|primary|range|raw|refresh|replace|respect|return|returning|returns|right|rollup|row|rows|rtree|run|satisfies|scope|secondary|select|set|sets|some|start|stop|synonym|temporary|then|ties|to|type|unbounded|union all|union|unknown|unnest|update|upsert|use|using|value|valued|when|where|with|write"
  );
