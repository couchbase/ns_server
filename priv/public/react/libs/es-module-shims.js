...
(function () {
  ...
  const esmsInitOptions$1 = optionsScript
  ...
  const skip = esmsInitOptions$1.skip
    ? new RegExp(esmsInitOptions$1.skip)
    : null;
  ...
  const fetchHook = esmsInitOptions$1.fetch
    ? globalHook(esmsInitOptions$1.fetch)
    : fetch;
  ...
  const enable = Array.isArray(esmsInitOptions$1.polyfillEnable)
    ? esmsInitOptions$1.polyfillEnable
    : [];
  ...
  const featureDetectionPromise = Promise.resolve(
    ...
    return Promise.all([
      ...
      cssModulesEnabled &&
        ...
        ).then(() => (supportsCssAssertions = true), noop),
      jsonModulesEnabled &&
        dynamicImport(
  ...
  const resolve = resolveHook
    ? async (id, parentUrl) => ({
        r: await esmsInitOptions.resolve(id, parentUrl, defaultResolve),
        b: false,
      })
    : _resolve;
  ...
  const initPromise = featureDetectionPromise.then(() => {
    ...
    if (!shimMode) {
      ...
      for (const script of document.querySelectorAll(
        ...
        if (!seenScript && script.type === 'module') seenScript = true;
        ...
        for (const mutation of mutations) {
          ...
          for (const node of mutation.addedNodes) {
            if (node.tagName === 'SCRIPT') {
              if (
                (!shimMode && node.type === 'module') ||
                (shimMode && node.type === 'module-shim')
              )
                processScript(node);
              if (
                (!shimMode && node.type === 'importmap') ||
                (shimMode && node.type === 'importmap-shim')
              )
  ...
  async function topLevelLoad(
    ...
    const module = await dynamicImport(
      !shimMode && !load.n && nativelyLoaded ? load.u : load.b,
      { errUrl: load.u }
    );
