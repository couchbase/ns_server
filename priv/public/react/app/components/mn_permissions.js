...
const bucketSpecificPermissions = [function (name, buckets) {
  var basePermissions = [
    ...
    if (resp.data["cluster.bucket[.].settings!read"]) {
      return mnBucketsService.getBucketsByType().then(function (bucketsDetails) {
        ...
        return doCheck(permissions).then(function (resp) {
          ...
          bucketsDetails.forEach(function (bucket) {
            let interesting = generateBucketPermissions(bucket.name, bucketsDetails);
            interesting.forEach(function (permission) {
              let bucketPermission = permission.split("[" + bucket.name + "]")[1];
              let collectionPermission = permission.split("[" + bucket.name + ":.:.]")[1];
...
function doCheck(interestingPermissions) {
  return axios.post('/pools/default/checkPermissions', interestingPermissions.join(','));
}
