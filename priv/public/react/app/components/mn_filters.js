    ...
    if (_.isArray(toSerialize)) {
      ...
      parts.push(
        encodeURIComponent(prefix) +
          '=' +
          encodeURIComponent(serializeValue(toSerialize))
      );
    }
  }
...
function mnFormatStorageMode(value, isEnterprise) {
  switch (value) {
    case 'plasma':
      return 'Standard GSI';
    case 'forestdb':
      return (isEnterprise ? 'Legacy' : 'Standard') + ' GSI';
    case 'memory_optimized':
      return 'Memory Optimized GSI';
    default:
      return value;
  }
}
...
function mnTruncateTo3Digits() {
  return function (value, leastScale, roundMethod) {
    ...
    var scale =
      _.detect([100, 10, 1, 0.1, 0.01, 0.001], function (v) {
        return value >= v;
      }) || 0.0001;
...
function mnFormatQuantity(value, numberSystem, spacing) {
  ...
  return [
    mnTruncateTo3Digits(value / t[0], undefined, 'floor'),
    spacing,
    t[1],
  ].join('');
}
