...
class MnSelect extends MnLifeCycleHooksToStream {
  ...
  componentWillMount() {
    ...
    function getPreparedValues() {
      vm.preparedValues = vm.props.labels
        ? vm.props.labels
        : (vm.props.values || []).map(vm.valuesMapping);
    ...
    function hasSearchInput() {
      return (
        (vm.props.hasSearch &&
          (vm.props.values || []).length >= searchMinimumOptionsNumber) ||
        false
      );
    }
  }
    ...
    return (
      <div className={`${className} uib-dropdown-menu`}>
        ...
        >
          ...
          <Dropdown.Menu
            className="panel absolute fit-content-width margin-0"
          ...
          >
            ...
            <div className="scrollable">
              ...
              {vm.hasSearchInput() &&
                ...
                vm
                  .getPreparedValues()
                  .filter((val) => val.includes(mnSearchValue))
                  .map((value, index) => (
                    <div key={index} className="block option">
                      <input
                        type="radio"
                        ...
                        checked={
                          vm.props.value ===
                          vm.props.values[vm.preparedValues.indexOf(value)]
                        }
                        onChange={() =>
                          vm.optionClicked(
                            vm.props.values[vm.preparedValues.indexOf(value)]
              ...
              {vm.hasSearchInput() &&
                labels &&
                labels
                  .filter((label) => label.includes(mnSearchValue))
                  .map((label, index) => (
                    <div key={index} className="block option">
                      <input
                        type="radio"
                        ...
                        checked={
                          vm.props.value ===
                          vm.props.values[labels.indexOf(label)]
                        }
                        onChange={() =>
                          vm.optionClicked(
                            vm.props.values[labels.indexOf(label)]
