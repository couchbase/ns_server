...
class MnTasksServiceClass {
  constructor() {
    this.stream = {};
    this.stream.tasksXDCRPlug = new BehaviorSubject();
    this.stream.tasksXDCR = this.stream.tasksXDCRPlug.pipe(
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.taskCollectInfoPlug = new BehaviorSubject();
    this.stream.taskCollectInfo = this.stream.taskCollectInfoPlug.pipe(
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.tasksWarmingUpPlug = new BehaviorSubject();
    this.stream.tasksWarmingUp = this.stream.tasksWarmingUpPlug.pipe(
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.tasksBucketCompactionPlug = new BehaviorSubject();
    this.stream.tasksCompactionByBucket =
      this.stream.tasksBucketCompactionPlug.pipe(
    ...
    this.stream.tasksLoadingSamples = this.stream.tasksLoadingSamplesPlug.pipe(
      filter((v) => v !== undefined),
      shareReplay({ refCount: true, bufferSize: 1 })
    );
  }
...
const MnTasksService = new MnTasksServiceClass();
