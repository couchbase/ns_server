...
class MnServicesConfigComponent extends MnLifeCycleHooksToStream {
  ...
  componentWillMount() {
    ...
    this.postPoolsDefaultValidationError = this.postPoolsDefaultValidation.error;
    ...
    if (this.props.isFlagEnabled && this.props.isFieldEnabled) {
      this.total =
        merge(MnHelperReactService.valueChanges(this.props.group.valueChanges), this.props.initDataStream)
  ...
  activateHotKeys() {
    ...
    isPressed
      .pipe(switchMap(() =>
            MnHelperReactService.valueChanges(this.props.group.get("flag").valueChanges).pipe(takeUntil(isNotPressed))),
            takeUntil(this.mnOnDestroy))
      .subscribe(flag => {
  ...
  render() {
    ...
    const { postPoolsDefaultValidationError, postClusterInitHttpError, mnServices, total } = this.state;
        ...
        <div>
          ...
          {mnServices?.map(service => (
            <div key={service}>
              <div className="row formrow">
                {group.value.flag && (
                  <div className="width-6">
                    <FieldControl strict={false} name={`flag.${service}`} render={({ handler }) => (
                      <>
                        <input
                          type="checkbox"
                          id={`${service}-ram-flag`}
                ...
                {group.value.field && service !== 'backup' && (
                  <div className="row width-6 flex-right relative">
                    <FieldControl strict={false} name={`field.${service}`} render={({ handler }) => (
                      <input
              ...
              <div
                className="error"
                hidden={!postPoolsDefaultValidationError?.errors?.[this.getServiceErrorName(service)]}>
                {postPoolsDefaultValidationError?.errors?.[this.getServiceErrorName(service)]}
              </div>
              <div
                className="error"
                hidden={!postClusterInitHttpError?.errors?.[this.getServiceErrorName(service)]}>
                {postClusterInitHttpError?.errors?.[this.getServiceErrorName(service)]}
              </div>
            </div>
          ))}
