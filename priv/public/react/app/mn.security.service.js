...
class MnSecurityServiceClass {
  constructor(http, mnAdminService, mnPoolsService, mnPermissions) {
    ...
    this.stream.getSaslauthdAuth = new BehaviorSubject().pipe(
      switchMap(this.getSaslauthdAuth.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.getCertificate = new BehaviorSubject().pipe(
      switchMap(this.getCertificate.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.getSettingsSecurity = new BehaviorSubject().pipe(
      switchMap(this.getSettingsSecurity.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.getLogRedaction = new BehaviorSubject().pipe(
      switchMap(this.getLogRedaction.bind(this))
    );

    this.stream.getClusterEncryption = this.stream.getSettingsSecurity.pipe(
      pluck('clusterEncryptionLevel')
    );

    this.stream.getClientCertAuth = new BehaviorSubject().pipe(
      switchMap(this.getClientCertAuth.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.getAudit = new BehaviorSubject().pipe(
      switchMap(this.getAudit.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.getUserActivity = new BehaviorSubject().pipe(
      switchMap(this.getUserActivity.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.getUIUserRoles = new BehaviorSubject().pipe(
      switchMap(this.getUIUserRoles.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );
    this.stream.getUIUserGroups = new BehaviorSubject().pipe(
      switchMap(this.getUIUserGroups.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.getSaml = new BehaviorSubject().pipe(
      switchMap(this.getSaml.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.getAuditDescriptors = new BehaviorSubject().pipe(
      switchMap(this.getAuditDescriptors.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.getAuditNonFilterableDescriptors = new BehaviorSubject().pipe(
      switchMap(this.getAuditNonFilterableDescriptors.bind(this)),
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    this.stream.postLogRedaction = new MnHttpRequest(
      this.postLogRedaction.bind(this)
    )
      .addSuccess()
      .addError();

    this.stream.postSettingsSecurity = new MnHttpRequest(
      this.postSettingsSecurity.bind(this)
    )
      .addSuccess()
      .addError();

    this.stream.postClientCertAuth = new MnHttpRequest(
      this.postClientCertAuth.bind(this)
    )
      .addSuccess()
      .addError();

    this.stream.postAuditValidation = new MnHttpRequest(
      this.postAudit(true).bind(this)
    )
      .addSuccess()
      .addError();

    this.stream.postAudit = new MnHttpRequest(this.postAudit(false).bind(this))
      .addSuccess()
      .addError();

    this.stream.postUserActivityValidation = new MnHttpRequest(
      this.postUserActivity(true).bind(this)
    )
      .addSuccess()
      .addError();

    this.stream.postUserActivity = new MnHttpRequest(
      this.postUserActivity(false).bind(this)
    )
      .addSuccess()
      .addError();

    this.stream.postSaml = new MnHttpRequest(this.postSaml(false).bind(this))
      .addSuccess()
      .addError();

    this.stream.postSamlValidation = new MnHttpRequest(
      this.postSaml(true).bind(this)
    )
      .addSuccess()
      .addError();

    this.stream.prepareOtherSettingsFormValues = combineLatest([
      isEnterprise,
      compatVersion55,
      settingsReadStream,
    ]).pipe(
      switchMap(this.getOtherSettingsSources.bind(this)),
  ...
  getAuditNonFilterableDescriptors() {
    return this.http.get('/settings/audit/nonFilterableDescriptors').pipe(
      map((data) =>
        data.map((desc) => {
  ...
  getOtherSettingsSources([isEnterprise, compatVersion55, settingsRead]) {
    let sources = [this.mnAdminService.stream.uiSessionTimeout];
    if (isEnterprise) {
...
const MnSecurityService = new MnSecurityServiceClass(
  HttpClient,
  MnAdminService,
  MnPoolsService,
  mnPermissions
