...
var mapping71to76 = {
  ...
  '@cbas-.cbas_failed_to_parse_records_count':
    '@cbas-.cbas_failed_to_parse_records_total',
};
    ...
    path.forEach((key, index) => {
      if (index == path.length - 1) {
        parent[key] = Object.assign(
          ...
          readByPath(
            statPath,
            !mapping65[statPath] ? compat70.stats : compat65.stats
          ),
          config
        );
      } else {
...
var compat72Combined = propertiesToArray(compat72.stats).reduce(
  (acc, statPath) => {
    ...
    path.forEach((key, index) => {
      if (index == path.length - 1) {
        parent[key] = Object.assign(
          ...
          readByPath(statPath, compat72.stats),
          config
        );
      } else {
        parent[key] = parent[key] || {};
...
var compat764Combined = propertiesToArray(compat764.stats).reduce(
  (acc, statPath) => {
    let config = getStatAdditionalConfig(statPath);
    let path = statPath.split('.');
    let parent = acc;
    path.forEach((key, index) => {
      if (index == path.length - 1) {
        parent[key] = Object.assign(
          ...
          readByPath(statPath, compat764.stats),
          config
        );
      } else {
        parent[key] = parent[key] || {};
...
function get76Stats() {
  ...
  for (let category in compat72Combined) {
    ...
    for (let stat in compat72Combined[category]) {
      ...
      Object.assign(
        compat76Combined[category][stat],
        compat72Combined[category][stat]
      );
    }
  }
  ...
  for (let category in compat76.stats) {
    for (let stat in compat76.stats[category]) {
      ...
      compat76Combined[category][statName] =
        compat76Combined[category][statName] || {};
      Object.assign(compat76Combined[category][statName], statData);
      Object.assign(
        compat76Combined[category][statName],
        getStatAdditionalConfig(`${category}.${statName}`)
      );
...
function getStatAdditionalConfig(statName) {
  switch (statName) {
    case '@kv-.kv_ep_active_ahead_exceptions':
      return {
        metric: { name: 'kv_ep_ahead_exceptions', state: 'active' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_ep_replica_ahead_exceptions':
      return {
        metric: { name: 'kv_ep_ahead_exceptions', state: 'replica' },
        applyFunctions: ['irate'],
      };

    case '@kv-.kv_vb_avg_replica_queue_age_seconds':
      return {
        metric: { name: 'kv_vb_avg_queue_age_seconds', state: 'replica' },
        nodesAggregation: 'special',
      };
    case '@kv-.kv_vb_avg_active_queue_age_seconds':
      return {
        metric: { name: 'kv_vb_avg_queue_age_seconds', state: 'active' },
        nodesAggregation: 'special',
      };
    case '@kv-.kv_vb_avg_pending_queue_age_seconds':
      return {
        metric: { name: 'kv_vb_avg_queue_age_seconds', state: 'pending' },
        nodesAggregation: 'special',
      };

    case '@kv-.kv_vb_active_eject':
      return {
        metric: { name: 'kv_vb_eject', state: 'active' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_active_ops_create':
      return {
        metric: { name: 'kv_vb_ops_create', state: 'active' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_active_queue_drain':
      return {
        metric: { name: 'kv_vb_queue_drain', state: 'active' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_active_queue_fill':
      return {
        metric: { name: 'kv_vb_queue_fill', state: 'active' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_active_sync_write_accepted_count':
      return {
        metric: { name: 'kv_vb_sync_write_accepted_count', state: 'active' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_active_sync_write_committed_count':
      return {
        metric: { name: 'kv_vb_sync_write_committed_count', state: 'active' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_active_sync_write_aborted_count':
      return {
        metric: { name: 'kv_vb_sync_write_aborted_count', state: 'active' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_pending_eject':
      return {
        metric: { name: 'kv_vb_eject', state: 'pending' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_pending_ops_create':
      return {
        metric: { name: 'kv_vb_ops_create', state: 'pending' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_pending_queue_drain':
      return {
        metric: { name: 'kv_vb_queue_drain', state: 'pending' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_pending_queue_fill':
      return {
        metric: { name: 'kv_vb_queue_fill', state: 'pending' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_replica_eject':
      return {
        metric: { name: 'kv_vb_eject', state: 'replica' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_replica_ops_create':
      return {
        metric: { name: 'kv_vb_ops_create', state: 'replica' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_replica_queue_drain':
      return {
        metric: { name: 'kv_vb_queue_drain', state: 'replica' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_vb_replica_queue_fill':
      return {
        metric: { name: 'kv_vb_queue_fill', state: 'replica' },
        applyFunctions: ['irate'],
      };
    ...
    case '@kv-.kv_vb_active_meta_data_memory_bytes':
      return {
        metric: { name: 'kv_vb_meta_data_memory_bytes', state: 'active' },
      };
    ...
    case '@kv-.kv_vb_pending_meta_data_memory_bytes':
      return {
        metric: { name: 'kv_vb_meta_data_memory_bytes', state: 'pending' },
      };
    ...
    case '@kv-.kv_vb_replica_meta_data_memory_bytes':
      return {
        metric: { name: 'kv_vb_meta_data_memory_bytes', state: 'replica' },
      };

    case '@kv-.kv_avg_active_timestamp_drift_seconds':
      return {
        metric: { name: 'kv_avg_timestamp_drift_seconds', state: 'active' },
        nodesAggregation: 'special',
      };
    case '@kv-.kv_avg_replica_timestamp_drift_seconds':
      return {
        metric: { name: 'kv_avg_timestamp_drift_seconds', state: 'replica' },
        nodesAggregation: 'special',
      };
    case '@kv-.kv_vb_pending_resident_items_ratio':
      return {
        metric: { name: 'kv_vb_resident_items_ratio', state: 'pending' },
        nodesAggregation: 'special',
      };
    case '@kv-.kv_vb_active_resident_items_ratio':
      return {
        metric: { name: 'kv_vb_resident_items_ratio', state: 'active' },
        nodesAggregation: 'special',
      };
    case '@kv-.kv_vb_replica_resident_items_ratio':
      return {
        metric: { name: 'kv_vb_resident_items_ratio', state: 'replica' },
        nodesAggregation: 'special',
      };
    ...
    case '@system.sysproc_mem_resident_java':
      return {
        metric: { name: 'sysproc_mem_resident', proc: 'java' },
        applyFunctions: ['sum'],
      };
    case '@system.sysproc_mem_resident_cbas':
      return {
        metric: { name: 'sysproc_mem_resident', proc: 'cbas' },
        applyFunctions: ['sum'],
      };

    case '@system.sysproc_mem_resident_java_cbas':
      return {
        metric: { name: 'sysproc_mem_resident', proc: 'java|cbas' },
        applyFunctions: ['sum'],
      };
    ...
    case '@xdcr-.xdcr_changes_left_total':
      return {
        metric: { name: 'xdcr_changes_left_total' },
        applyFunctions: ['sum'],
        bucketLabel: 'sourceBucketName',
      };
    ...
    case '@cbas.cbas_http_requests_failed_400_total':
      return {
        metric: { name: 'cbas_http_requests_failed_total', code: '400' },
        applyFunctions: ['sum'],
      };

    case '@cbas.cbas_http_requests_failed_401_total':
      return {
        metric: { name: 'cbas_http_requests_failed_total', code: '401' },
        applyFunctions: ['sum'],
      };

    case '@cbas.cbas_http_requests_failed_403_total':
      return {
        metric: { name: 'cbas_http_requests_failed_total', code: '403' },
        applyFunctions: ['sum'],
      };

    case '@cbas.cbas_http_requests_failed_404_total':
      return {
        metric: { name: 'cbas_http_requests_failed_total', code: '404' },
        applyFunctions: ['sum'],
      };

    case '@cbas.cbas_http_requests_failed_405_total':
      return {
        metric: { name: 'cbas_http_requests_failed_total', code: '405' },
        applyFunctions: ['sum'],
      };

    case '@cbas.cbas_http_requests_failed_409_total':
      return {
        metric: { name: 'cbas_http_requests_failed_total', code: '409' },
        applyFunctions: ['sum'],
      };

    case '@cbas.cbas_http_requests_failed_413_total':
      return {
        metric: { name: 'cbas_http_requests_failed_total', code: '413' },
        applyFunctions: ['sum'],
      };

    case '@cbas.cbas_http_requests_failed_500_total':
      return {
        metric: { name: 'cbas_http_requests_failed_total', code: '500' },
        applyFunctions: ['sum'],
      };

    case '@cbas.cbas_http_requests_failed_503_total':
      return {
        metric: { name: 'cbas_http_requests_failed_total', code: '503' },
        applyFunctions: ['sum'],
      };

    case '@xdcr-.@items.xdcr_rate_replicated_docs_per_second':
      return {
        metric: { name: 'xdcr_docs_written_total' },
        applyFunctions: ['irate', 'sum'],
      };
    ...
    case '@kv-.kv_avg_disk_time_seconds_update':
      return {
        metric: { name: 'kv_avg_disk_time_seconds', op: 'update' },
        nodesAggregation: 'special',
        applyFunctions: ['irate'],
      };
    ...
    case '@kv-.kv_avg_disk_time_seconds_commit':
      return {
        metric: { name: 'kv_avg_disk_time_seconds', op: 'commit' },
        nodesAggregation: 'special',
        applyFunctions: ['irate'],
      };
    ...
    case '@eventing-.@items.eventing_processed_count_rate':
      return {
        applyFunctions: ['irate'],
        bucket: null,
        metric: { name: 'eventing_processed_count' },
        nodesAggregation: 'special',
      };
    case '@eventing-.@items.eventing_failed_count_rate':
      return {
        applyFunctions: ['irate'],
        bucket: null,
        metric: { name: 'eventing_failed_count' },
        nodesAggregation: 'special',
      };
    case '@eventing-.@items.eventing_timeout_count_rate':
      return {
        applyFunctions: ['irate'],
        bucket: null,
        metric: { name: 'eventing_timeout_count' },
      };
    ...
    case '@kv-.kv_cas_hits':
      return {
        metric: { name: 'kv_ops', op: 'cas', result: 'hit' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_cas_badval':
      return {
        metric: { name: 'kv_ops', op: 'cas', result: 'badval' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_cas_misses':
      return {
        metric: { name: 'kv_ops', op: 'cas', result: 'miss' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_decr_hits':
      return {
        metric: { name: 'kv_ops', op: 'decr', result: 'hit' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_decr_misses':
      return {
        metric: { name: 'kv_ops', op: 'decr', result: 'miss' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_delete_hits':
      return {
        metric: { name: 'kv_ops', op: 'delete', result: 'hit' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_delete_misses':
      return {
        metric: { name: 'kv_ops', op: 'delete', result: 'miss' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_incr_hits':
      return {
        metric: { name: 'kv_ops', op: 'incr', result: 'hit' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_incr_misses':
      return {
        metric: { name: 'kv_ops', op: 'incr', result: 'miss' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_get_hits':
      return {
        metric: { name: 'kv_ops', op: 'get', result: 'hit' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_get_misses':
      return {
        metric: { name: 'kv_ops', op: 'get', result: 'miss' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_cmd_get':
      return {
        metric: { name: 'kv_ops', op: 'get' },
        applyFunctions: ['irate', 'sum'],
      };
    case '@kv-.kv_cmd_set':
      return {
        metric: { name: 'kv_ops', op: 'set' },
        applyFunctions: ['irate', 'sum'],
      };
    ...
    case '@kv-.kv_ep_num_ops_del_meta':
      return {
        metric: { name: 'kv_ops', op: 'del_meta' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_ep_num_ops_get_meta':
      return {
        metric: { name: 'kv_ops', op: 'get_meta' },
        applyFunctions: ['irate'],
      };
    case '@kv-.kv_ep_num_ops_set_meta':
      return {
        metric: { name: 'kv_ops', op: 'set_meta' },
        applyFunctions: ['irate'],
      };
    ...
    default:
      if (statName.includes('@kv-.kv_dcp_')) {
        ...
        if (
          statName.includes('@kv-.kv_dcp_items_sent_') ||
          statName.includes('@kv-.kv_dcp_total_data_size_bytes_')
        ) {
          applyFunctions.push('irate');
        }
        if (
          statName.includes('@kv-.kv_dcp_backoff_cbas') ||
...
function get70Mapping() {
  ...
  return Object.assign(rv, {
    ...
    '@cbas-.cbas_incoming_records_count_total':
      '@cbas-.cbas/incoming_records_count_total',
    ...
    '@kv-.kv_vb_active_sync_write_accepted_count':
      '@kv-.vb_active_sync_write_accepted_count',
    '@kv-.kv_vb_active_sync_write_committed_count':
      '@kv-.vb_active_sync_write_committed_count',
    '@kv-.kv_vb_active_sync_write_aborted_count':
      '@kv-.vb_active_sync_write_aborted_count',
    ...
    '@kv-.kv_avg_active_timestamp_drift_seconds':
      '@kv-.avg_active_timestamp_drift',
    '@kv-.kv_avg_replica_timestamp_drift_seconds':
      '@kv-.avg_replica_timestamp_drift',
    ...
    '@xdcr-.@items.xdcr_docs_received_from_dcp_total':
      '@xdcr-.@items.rate_received_from_dcp',
    '@xdcr-.@items.xdcr_rate_replicated_docs_per_second':
      '@xdcr-.@items.rate_replicated',
    '@xdcr-.@items.xdcr_wtavg_docs_latency_seconds':
      '@xdcr-.@items.wtavg_docs_latency', //?s-ms
    '@xdcr-.@items.xdcr_wtavg_meta_latency_seconds':
      '@xdcr-.@items.wtavg_meta_latency', //?s-ms

    '@xdcr-.@items.xdcr_percent_completeness':
      '@xdcr-.@items.percent_completeness',
    ...
    '@fts.fts_total_queries_rejected_by_herder':
      '@fts.fts_total_queries_rejected_by_herder',
    '@fts.fts_curr_batches_blocked_by_herder':
      '@fts.fts_curr_batches_blocked_by_herder',
    ...
    '@eventing-.@items.eventing_timeout_count':
      '@eventing-.@items.timeout_count',
    '@eventing-.@items.eventing_processed_count':
      '@eventing-.@items.processed_count',
