...
class MnMultiChart extends MnLifeCycleHooksToStream {
  ...
  componentDidMount() {
    ...
    vm.chart = new mnD3Service.mnD3Tooltip(options, vm.chartRef.current, function () {
    ...
    function addFocusChart() {
      ...
      vm.chartF = new mnD3Service.mnD3Focus(focusChartOpt, vm.chartRef.current, vm.chart);
    ...
    function syncTooltipsAndPauseCharts() {
      ...
      let cancelStatsTimer = () => MnHelperReactService.rootScopeEmitter.emit("mnStatsCancelTimer");
      ...
      let throttledPause = _.throttle(function () {
        cancelStatsTimer();
      ...
      MnHelperReactService.rootScopeEmitter.on("mnStatsCancelTimer", function () {
        resumeStatsTimer && clearTimeout(resumeStatsTimer);
      });
      ...
      MnHelperReactService.rootScopeEmitter.on("syncTooltips", function (e) {
        ...
        if (source.element !== vm.chartRef.current) {
          ...
          vm.chart.tipBox.node().dispatchEvent(createEvent(
            source.event.type,
            clientX,
            clientY
          ));
