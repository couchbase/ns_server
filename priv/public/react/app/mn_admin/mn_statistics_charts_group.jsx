...
class MnStatisticsChartsGroup extends MnLifeCycleHooksToStream {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (statisticsNewCtl.eventingItems?.values[0] !== prevStatisticsNewCtl.eventingItems?.values[0] ||
        statisticsNewCtl.xdcrItems?.values[0] !== prevStatisticsNewCtl.xdcrItems?.values[0] ||
        statisticsNewCtl.indexItems?.values[0] !== prevStatisticsNewCtl.indexItems?.values[0] ||
        statisticsNewCtl.viewItems?.values[0] !== prevStatisticsNewCtl.viewItems?.values[0] ||
        statisticsNewCtl.ftsItems?.values[0] !== prevStatisticsNewCtl.ftsItems?.values[0]) {
      vm.onItemChange();
    }
  }
  ...
  componentWillMount() {
    ...
    function getGroup() {
      return statisticsNewCtl.state.groupsById && statisticsNewCtl.state.groupsById[groupID];
    }
        ...
        <div
          ...
          {vm.isDetailsOpened(groupID) && (
            <div 
              ...
              onClick={(e) => e.stopPropagation()}>
              {vm.enabledItems?.['@index-'] && statisticsNewCtl.state.indexItems?.values?.length && (
                <div className="row flex-wrap">
                  <strong className="margin-right-quarter">Indexes</strong>
                  <MnSelect
                    className="inline max-width-2"
                    value={vm.state.items?.index || statisticsNewCtl.state.indexItems?.values[0]}
              ...
              {vm.enabledItems?.['@kv-'] && statisticsNewCtl.state.viewItems?.values?.length && (
                <div className="row flex-wrap margin-left-1">
                  <strong className="margin-right-quarter">Views</strong>
                  <MnSelect
                    className="inline max-width-2"
                    value={vm.state.items?.kv || statisticsNewCtl.state.viewItems?.values[0]}
              ...
              {vm.enabledItems?.['@xdcr-'] && statisticsNewCtl.state.xdcrItems?.values?.length && (
                <div className="row flex-wrap margin-left-1">
                  <strong className="margin-right-quarter">Replications</strong>
                  <MnSelect
                    className="inline max-width-2"
                    value={vm.state.items?.xdcr || statisticsNewCtl.state.xdcrItems?.values[0]}
              ...
              {vm.enabledItems?.['@eventing-'] && statisticsNewCtl.state.eventingItems?.values?.length && (
                <div className="row flex-wrap margin-left-1">
                  <strong className="margin-right-quarter">Eventing</strong>
                  <MnSelect
                    className="inline max-width-2"
                    value={vm.state.items?.eventing || statisticsNewCtl.state.eventingItems?.values[0]}
              ...
              {vm.enabledItems?.['@fts-'] && statisticsNewCtl.state.ftsItems?.values?.length && (
                <div className="row flex-wrap margin-left-1">
                  <strong className="margin-right-quarter">Search Indexes</strong>
                  <MnSelect
                    className="inline max-width-2"
                    value={vm.state.items?.fts || statisticsNewCtl.state.ftsItems?.values[0]}
        ...
        {vm.isDetailsOpened(groupID) && (
          <div className="row charts">
            {group.charts.map((chartID) => {
              ...
              return <MnStatisticsChartsChart
