...
function mnD3ServiceFactory() {
  class mnD3 {
    constructor(options, rootElement) {
      ...
      this.svg = root
        ...
        .attr(
          'transform',
          this.getTransform(this.cht.margin.left, this.cht.margin.top)
        );
    ...
    init() {
      ...
      this.xAxis = axisBottom()
        .scale(this.xScale)
        .tickFormat(this.cht.xAxis.tickFormat);
      this.svg
        .append('g')
        .attr('transform', this.getTransform(0, this.cvsRect.height))
        .attr('class', 'xAxis');
      ...
      if (this.cht.yAxis[1]) {
        this.createYAxis(
          1,
          axisRight,
          this.getTransform(this.cvsRect.width, 0)
        );
      }
    }
    destroy() {
    ...
    createYAxis(i, axis, coordinates) {
      ...
      this.yAxis[i] = axis()
        .scale(this.yScale[i])
        .tickFormat(this.cht.yAxis[i].tickFormat);
    ...
    updateData(data) {
      ...
      if (this.opt.is70Cluster) {
        ...
        this.xAxisMap = data.map((line) =>
          line.values.reduce((acc, [ts, v]) => {
            acc[ts] = v;
            return acc;
          }, {})
        );
        ...
        this.xAxisData = data.reduce(
          (max, item) => (item.values.length > max.length ? item.values : max),
          []
        );
    ...
    showEmptyContent() {
      ...
      this.svg.html(
        "<text class='charts-nodata'>" + this.cht.noData + '</text>'
      );
    }
    getXaxisDomain(data) {
      if (this.opt.is70Cluster) {
    ...
    resize() {
      ...
      if (this.cht.yAxis[1] && !this.cht.hideTicks) {
        this.svg
          .select('.yAxis1')
          .attr('transform', this.getTransform(this.cvsRect.width, 0));
      }
      this.updateData(this.data);
    }
  }
  ...
  class mnD3Focus extends mnD3 {
    ...
    init() {
      ...
      if (this.opt.is70Cluster) {
        ...
        this.bisect = bisector(function (d) {
          return d[0];
        }).left;
      }
      ...
      this.brush = brushX()
        .extent([
          [0, 0],
          [this.cvsRect.width, this.cht.height],
        ])
        .on('brush end', this.brushed.bind(this));

      this.brushEl = this.svg.append('g').attr('class', 'charts-brush');

      this.svg.attr('class', 'focus-chart');

      this.brushEl.call(this.brush).call(this.brush.move, null);
    ...
    getDomain() {
      ...
      return s
        ? s.map(this.xScale.invert, this.xScale)
        : this.getXaxisDomain(data);
    }
    brushed() {
      if (!this.data) {
        return;
    ...
    resize() {
      ...
      if (brushSelection(this.brushEl.node())) {
        //proportional resize of selection
        let data = this.opt.is70Cluster ? this.xAxisTimestamps : this.xAxisData;
  ...
  class mnD3Tooltip extends mnD3 {
    ...
    init() {
      ...
      this.drawTooltipThrottle = _.throttle(this.drawTooltip.bind(this), 10, {
        leading: true,
      });

      this.tipBox
        .node()
        .addEventListener('mousemove', this.setMouseMoveEvent.bind(this));
      this.tipBox
        .node()
        .addEventListener('mousemove', this.drawTooltipThrottle);
      this.tipBox
        .node()
        .addEventListener('mouseout', this.hideTooltip.bind(this));
      if (this.opt.isPauseEnabled) {
        this.tipBox
          .node()
          .addEventListener('mousemove', this.cancelLineAnimation.bind(this));
      }
    ...
    toggleLine(i) {
      ...
      select(this.getLegends.bind(this)().nodes()[i]).classed(
        'disabled',
        this.data[i].disabled
      );
    }
    resize() {
      super.resize();
  ...
  function drawCircle(path) {
    path
      ...
      .call(
        this.updateCirclePosition.bind(this),
  ...
  function drawTooltip() {
    ...
    if (this.opt.is70Cluster) {
      ...
      circlesPerLine = this.tipLineWrap
        .selectAll('.circle-per-line')
        .data(this.data)
        .style(
          'opacity',
          function (d, i) {
      ...
      this.selectedValueIndex =
        !d0 || xDate - d0[0] > d1[0] - xDate ? i : i - 1;
      ...
      this.svg
        ...
        .attr(
          ...
          function () {
            ...
            var d =
              'M' +
              this.xScale(this.xAxisData[idx][0]) +
              ',' +
              elementRect.height;
        ...
        .style(
          ...
          function (d) {
            ...
            return d.values.length &&
              !d.disabled &&
              !isNaN(d.values[idx] && d.values[idx][1])
              ? 1
              : 0;
          }.bind(this)
        );
    ...
    circlesPerLine.join(
      ...
      function (update) {
        update.transition().duration(0).call(this.drawCircle.bind(this));
      }.bind(this)
    );
    ...
    if (!this.disableTooltipFlag) {
      var tooltipRows = this.tip
        ...
        .style(
          leftRight,
          (leftRight == 'left' ? this.mouseMoveEvent.pageX + 30 : rightPos) +
            'px'
        )
        .style(
          topBottom,
          (topBottom == 'top' ? this.mouseMoveEvent.pageY + 20 : bottomPos) +
            'px'
        )
        .selectAll('.charts-tooltip-row')
        .data(
          this.data.map(this.getLabelRowValues.bind(this)),
          (line) =>
            line.color + line.key + line.value + line.yAxis + line.disabled
        );
  ...
  function getLabelRowValues(line, i) {
    ...
    if (this.opt.is70Cluster) {
      ...
      value =
        !line.values[idx] || line.values[idx][1] == undefined
          ? '-'
  ...
  function drawLegends() {
    //Legends
    this.legendsWrap = this.rootEl
      .append('div')
      .attr('class', 'legends-wrap')
      .append('div')
      .attr('class', 'charts-filter-icon');
    ...
    this.getLegends()
      ...
      .join(
        ...
        function (update) {
          update.html(getLegendsHtml.bind(this));
        }.bind(this)
      );

    this.clickCB = this.getLegends()
      .nodes()
      .map(
        function (node, i) {
          var cb = function () {
  ...
  function updateCirclePosition(pipe, idx) {
    return this.opt.is70Cluster
      ? pipe.attr('transform', (line, i) => {
          let selectedValue = this.xAxisMap[i][this.selectedXDate];
      ...
      : pipe.attr(
          ...
          function (line) {
            if (
              line.values[idx] &&
              line.values[idx].length &&
              !isNaN(line.values[idx][1])
            ) {
              return this.getTransform(
                this.xScale(line.values[idx][0]),
...
const mnD3Service = mnD3ServiceFactory();
