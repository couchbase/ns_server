...
function mnGsiFooterController(
  $scope,
  $rootScope,
  $state,
  mnStatisticsNewService,
  ...
  vm.getLatestStat = mnPoolDefault.export.compat.atLeast70
    ? getLatestStat70
    : getLatestStat;

  vm.currentBucket =
    $state.params.footerBucket ||
  ...
  vm.stats =
    ...
    (mnPoolDefault.export.compat.atLeast70
      ? [
          '@index.index_memory_quota',
          '@index.index_memory_used_total',
          '@index-.index_num_rows_returned',
          '@index-.index_fragmentation',
  ...
  function doGetLatestStat70(statName) {
    ...
    return (
      stats &&
      stats[statName] &&
      stats[statName].aggregate &&
      Number(
        stats[statName].aggregate.values
          .slice()
          .reverse()
  ...
  function getLatestStat70(statName) {
    switch (statName) {
      ...
      case '@index.index_ram_percent': {
        return (
          (doGetLatestStat70('@index.index_memory_used_total') /
            doGetLatestStat70('@index.index_memory_quota')) *
          100
        );
      }
      default:
  ...
  function getLatestStat(statName) {
    ...
    return (
      ...
      $scope.mnUIStats.stats[statName].aggregate
        .slice()
        .reverse()
        .find((stat) => stat != null)
    );
