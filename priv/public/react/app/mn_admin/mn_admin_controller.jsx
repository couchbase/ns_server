...
class MnAdminComponent extends MnLifeCycleHooksToStream {
  ...
  componentDidMount() {
    ...
    if (mnPermissions.export.getValue().cluster.tasks.read) {
      if (vm.props.pools.isEnterprise && vm.props.poolDefault.compat.atLeast65) {
      ...
      var tasksPoller = new mnPoller($scope, function (prevTask) {
        ...
        MnHelperReactService.rootScopeEmitter.on("reloadTasksPoller", function (event, params) {
        ...
        MnHelperReactService.rootScopeEmitter.on("reloadBucketStats", function () {
        ...
        <div className="alert-wrapper fix-position-bl">
          ...
          {vm.state.alerts.map((alert, index) => (
            <div
              ...
              {alert.type !== 'success' && (
                <a
                  onClick={() => vm.closeAlert(alert)}
                  className="close">X</a>
              )}
            </div>
          ))}
        </div>
        <div className={`main-content min-width-zero delayed-spinner expanded-spinner fixed-spinner width-12 ${mainSpinnerCounter ? 'mn-main-spinner-active' : ''}`}>
