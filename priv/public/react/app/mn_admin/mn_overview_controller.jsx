...
class MnOverviewComponent extends MnLifeCycleHooksToStream {
  ...
  componentWillMount() {
    ...
    vm.addressFamilyOnly =
      mnPoolDefault.export.getValue().thisNode.addressFamilyOnly;
    ...
    function activate() {
      new mnPoller(vm, function () {
        return mnServersService.getServicesStatus(
          mnPoolDefault.export.getValue().isEnterprise
        );
      })
      ...
      <>
            ...
            <div className="zero-content">
              ...
              {rbac.cluster.buckets.create && (
                <span>
                  ...
                  to add one, or load a sample bucket with data & indexes from
                  Settings {'>'}{' '}
                  <UISref to="app.admin.settings.sampleBuckets">
                    <a>Sample Buckets</a>
                  </UISref>
                  .
                </span>
        ...
        <footer className="footer-dashboard">
          <div className="service-widget-row resp-margin-xsml">
            ...
            >
              <p>
                Data - {buckets.details.length} bucket
                {this.getEndings(buckets.details.length)}
              </p>
              {Object.entries(nodes?.kv.nodesByStatuses || {}).map(
                ([status, count]) => (
            ...
            >
              <p>Index</p>
              {Object.entries(nodes?.index.nodesByStatuses || {}).map(
                ([status, count]) => (
                  <div key={status} className="error piped">
                    {count} node{this.getEndings(count)} {status}
                  </div>
            ...
            <div
              ...
              onClick={() =>
                UIRouter.stateService.go('app.admin.query.monitoring')
              }
            >
              <p>Query</p>
              {Object.entries(nodes?.n1ql.nodesByStatuses || {}).map(
                ([status, count]) => (
                  <div key={status} className="error piped">
                    {count} node{this.getEndings(count)} {status}
                  </div>
            ...
            <div
              ...
              onClick={() =>
                UIRouter.stateService.go('app.admin.search.fts_list')
              }
            >
              <p>Search</p>
              {Object.entries(nodes?.fts.nodesByStatuses || {}).map(
                ([status, count]) => (
                  <div key={status} className="error piped">
                    {count} node{this.getEndings(count)} {status}
                  </div>
            ...
            {poolDefault.isEnterprise && (
              <div
                ...
                onClick={() =>
                  UIRouter.stateService.go('app.admin.cbas.workbench')
                }
              >
                <p>Analytics</p>
                {Object.entries(nodes?.cbas.nodesByStatuses || {}).map(
                  ([status, count]) => (
                    <div key={status} className="error piped">
                      {count} node{this.getEndings(count)} {status}
                    </div>
            ...
            {poolDefault.isEnterprise && (
              <div
                ...
                onClick={() =>
                  UIRouter.stateService.go('app.admin.eventing.summary')
                }
              >
                <p>Eventing</p>
                {Object.entries(nodes?.eventing.nodesByStatuses || {}).map(
                  ([status, count]) => (
                    <div key={status} className="error piped">
                      {count} node{this.getEndings(count)} {status}
                    </div>
            ...
            {rbac.cluster.xdcr.remote_clusters.read && (
              <div
                ...
                onClick={() =>
                  UIRouter.stateService.go('app.admin.replications')
                }
              >
                <p>XDCR</p>
              </div>
          ...
          <div className="dashboard-servers resp-xsml">
            <div className="dashboard-node" title="active nodes">
              ...
              <p>
                {nodes?.all.active.length || '0'} active node
                {this.getEndings(nodes?.all.active.length)}
              </p>
            </div>
            ...
            <div className="dashboard-node" title="failed-over nodes">
              ...
              <p>
                {nodes?.all.failedOver.length || '0'} failed-over node
                {this.getEndings(nodes?.all.failedOver.length)}
              </p>
            </div>
            ...
            <div className="dashboard-node" title="nodes pending rebalance">
              <span className="icon fa-server orange-2"></span>
              <p>
                {nodes?.all.pending.length || '0'} node
                {this.getEndings(nodes?.all.pending.length)} pending rebalance
              </p>
            </div>
            ...
            <div className="dashboard-node" title="inactive nodes">
              ...
              <p>
                {nodes?.all.down.length || '0'} inactive node
                {this.getEndings(nodes?.all.down.length)}
              </p>
            </div>
          </div>
        </footer>
