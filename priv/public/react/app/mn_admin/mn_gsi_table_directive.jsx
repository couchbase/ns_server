...
import {
  MnSortableTable,
  MnSortableTitle,
} from '../components/directives/mn_sortable_table.jsx';
import {
  MnGsiItemDetails,
  MnGsiItemController,
} from './mn_gsi_item_details.jsx';
...
class MnGsiTable extends MnLifeCycleHooksToStream {
  ...
  componentDidMount() {
    ...
    function getRowKeyspace(row) {
      return (
        row.bucket +
        (row.scope
          ? '.' + row.scope + (row.collection ? '.' + row.collection : '')
          : '')
      );
    }
    ...
    function getStatusDescription(row) {
      ...
      switch (row.status) {
        case 'Created':
          return 'Index definition has been saved. Use Build Index to build the index. It is NOT serving scan requests yet.';
        case 'Building':
          return 'Index is currently building. It is NOT serving scan requests yet.';
        case 'Ready':
          return 'Index is ready to serve scan requests.';
        case 'Replicating':
          return 'Index is being replicated as part of a Rebalance or Alter Index operation. It is NOT serving scan requests until replication is complete.';
        case 'Paused':
          return 'Index is not ingesting new mutations as allocated memory has been completely used.';
        case 'Warmup':
          return 'Index is being loaded from persisted on-disk snapshot after indexer process restart. It is NOT serving scan requests yet.';
        case 'Error':
          return 'Index is in an error state and cannot be used in scan operations.';
        case 'Created (Upgrading)':
          return 'Index definition has been upgraded from Legacy storage engine to Standard GSI. It is NOT serving scan requests yet.';
        case 'Created (Downgrading)':
          return 'Index definition has been downgraded from Standard GSI to Legacy storage engine. It is NOT serving scan requests yet.';
        case 'Building (Upgrading)':
          return 'Index is building after upgrade from Legacy storage engine to Standard GSI. It is NOT serving scan requests yet.';
        case 'Building (Downgrading)':
          return 'Index is building after downgrade from Standard GSI to Legacy storage engine. It is NOT serving scan requests yet.';
        case 'Not Available':
  ...
  render() {
    ...
    return (
      <MnSortableTable
        className="cbui-table"
        ref={vm.sortableTableProperties}
        setState={vm.setState.bind(vm)}
        state={vm.state}
      >
        <div className="cbui-table-header">
          <span className="cbui-table-cell flex-grow-1-5">
            <MnSortableTitle
              mnSortableTitle="index"
              sortByDefault="true"
              className="sorter"
            >
              index name
          ...
          <span className="cbui-table-cell flex-grow-half">items</span>
          <span className="cbui-table-cell flex-grow-half resp-hide-xsml">
            data size
          </span>
        ...
        {paginatorPage?.map((row) => (
          ...
          >
            <MnGsiItemController row={row}>
              <div
                className={`cbui-tablerow has-hover ${!vm.isDetailsOpened(vm.generateIndexId(row)) ? vm.getStatusClass(row) : ''}`}
              >
                <span
                  className="cbui-table-cell flex-grow-1-5"
                  title={row.index}
                >
                  {row.indexName || row.index}
                  <span className="flex-inline flex-wrap">
                    {row.partitioned && (
                      <span className="label lt-blue">partitioned</span>
                    )}
                    ...
                    {row.index.indexOf('(replica') > -1 && (
                      <OverlayTrigger
                        placement="auto"
                        overlay={
                          <Tooltip id="replicas-tooltip">
                            Index replicas are always active and automatically
                ...
                <span className="cbui-table-cell flex-grow-half resp-hide-xsml">
                  {vm.isFinite(row.num_requests)
                    ? mnFormatQuantity(row.num_requests, 1000)
                    : '-'}
                </span>
                <span className="cbui-table-cell flex-grow-half resp-hide-sml">
                  {vm.isFinite(row.index_resident_percent)
                    ? `${row.index_resident_percent}%`
                    : '-'}
                </span>
                <span className="cbui-table-cell flex-grow-half">
                  {vm.isFinite(row.items_count)
                    ? mnFormatQuantity(row.items_count, 1000)
                    : '-'}
                </span>
                <span className="cbui-table-cell flex-grow-half resp-hide-xsml">
                  {vm.isFinite(row.data_size)
                    ? mnFormatQuantity(row.data_size)
                    : '-'}
                </span>
                <span
                  className="cbui-table-cell"
                  title={vm.getRowKeyspace(row)}
                >
                ...
                <span className="cbui-table-cell flex-wrap text-smaller">
                  ...
                  {vm.isFinite(row['num_docs_pending+queued']) &&
                    row['num_docs_pending+queued'] !== 0 && (
                      <div className="label warning">
                        {mnFormatQuantity(row['num_docs_pending+queued'], 1000)}{' '}
                        mutations remaining
                      </div>
                    )}
                </span>
            ...
            {vm.isDetailsOpened(vm.generateIndexId(row)) && (
              <MnGsiItemDetails
        ...
        {paginatorPage?.length === 0 && (
          <div className="zero-content margin-top-1-5">
            No indexes found for this bucket.scope combination. Try another
            bucket.scope.
          </div>
        )}
        <div className="row">
