...
function mnSettingsClusterServiceFactory(mnPools, mnPoolDefault) {
  ...
  function postSettingsResource(data) {
    return axios.post('/settings/resourceManagement', {
      'diskUsage.enabled': data.diskUsage.enabled,
      'diskUsage.maximum': data.diskUsage.maximum,
    });
  }
  ...
  function getSettingsRetryRebalance() {
    return axios.get('/settings/retryRebalance').then(function (resp) {
      return resp.data;
    });
  }
  ...
  function postCancelRebalanceRetry(replicationId) {
    return axios({
      url:
        '/controller/cancelRebalanceRetry/' + encodeURIComponent(replicationId),
      method: 'POST',
      mnHttp: { group: 'global' },
    });
  }
  ...
  function getSettingsAnalytics() {
    return axios.get('/settings/analytics').then(function (resp) {
      return resp.data;
    });
  }
...
const mnSettingsClusterService = mnSettingsClusterServiceFactory(
  mnPools,
  mnPoolDefault
);
