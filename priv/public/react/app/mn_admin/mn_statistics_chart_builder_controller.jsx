...
class MnStatisticsChartBuilderComponent extends MnLifeCycleHooksToStream {
  ...
  componentWillMount() {
    ...
    function reActivateStats() {
      ...
      Object.keys(getSelectedStats()).forEach((descPath) => onStatChecked(descPath, true));
    }
    ...
    function maybeDisableField(descPath) {
      ...
      return ((vm.state.newChart.specificStat == "false") &&
              vm.state.disableStats && !vm.units[stat.unit]) ||
    ...
    function onSpecificChecked(value) {
      if (value == "true") {
        ...
        vm.setState(state => ({
          newChart: {...state.newChart, specificStat: "true", stats: selectedByNodeStats}
        }));
      } else {
        selectedByNodeStats = Object.assign({}, vm.state.newChart.stats);
        vm.setState(state => ({
          newChart: {...state.newChart, specificStat: "false", stats: selectedStats}
        }));
      }
    ...
    function onStatChecked(descPath, value) {
      ...
      if (!desc) {
        vm.setState(state => ({
          newChart: {...state.newChart, stats: {...state.newChart.stats, [descPath]: false}}
        }));
      ...
      vm.setState(state => ({
        disableStats: vm.state.newChart.specificStat !== "false" ? selectedUnitsCount >= 1 : selectedUnitsCount >= 2,
        newChart: {...state.newChart, stats: {...state.newChart.stats, [descPath]: value}}
      }));
    }
    ...
    function activate() {
      if (vm.isEditing) {
        ...
        Object.keys(newChart.stats).forEach((descPath) => onStatChecked(descPath, true));
      } else {
        vm.setState({
          newChart: {
            stats: {},
      ...
      if (vm.isEditing) {
        vm.setState({
          tab: Object.keys(chart.stats).map(function (stat) {
        ...
        vm.setState({
          selectedKVFilters: Object.keys(chart.stats).filter(function (stat) {
    ...
    function create() {
      ...
      var toGroup = mnStoreService.store("groups").get(vm.state.selectedGroup || group.id);
      var fromGroup;
      if (vm.isEditing) {
        ...
        <form 
          ...
          <div className="panel-content" style={{paddingBottom: 0}}>
            <div className="row items-top content-box min-padding margin-bottom-half">
              ...
              <div className="margin-right-1 nowrap">
                ...
                <MnSelect
                  ...
                  value={newChart.size}
                  onSelect={({selectedOption}) => vm.setState(state => ({
              ...
              {vm.isEditing && (
                <div className="nowrap">
                  ...
                  <MnSelect
                    ...
                    value={selectedGroup}
                    onSelect={({selectedOption}) => vm.setState({selectedGroup: selectedOption})}
                    values={vm.groupIds}
            ...
            <div className="row pills margin-bottom-half">
              {vm.tabs.map(name => (
                ((name !== '@eventing' && name !== '@cbas') || vm.props.poolDefault.isEnterprise) && (
                  <a
                    key={name}
                    className={`margin-right-half ${tab === name ? 'selected' : ''}`}
                    onClick={() => vm.selectTab(name)}>
                    {vm.mnFormatServicesFilter(mnFormatStatsSectionsFilter(name))}
                  </a>
                )
              ))}
            </div>
            ...
            <div className="scrolling-wrapper">
              <div className="margin-bottom-1-5 margin-right-half columns-3">
                ...
                {tab === '@kv' && (
                  <div>
                    {Object.entries(vm.kvGroups).map(([name1, group]) => (
                      <div key={name1}>
                        {group.map(name => {
                          const isDisabled = vm.maybeDisableField('@kv-.' + name);
                          const hasDesc = vm.statsDesc['@kv-'][name]?.desc;
                          if (!selectedKVFilters[name1] || !hasDesc) return null;
                          ...
                          return (
                            <div 
                              ...
                              <input
                                ...
                                type="checkbox"
                                checked={newChart.stats['@kv-.' + name] || false}
                                onChange={(e) => vm.onStatChecked('@kv-.' + name, e.target.checked)}
                                disabled={isDisabled} />
                              <OverlayTrigger
                                placement="auto-end"
                ...
                {vm.statsDesc[tab + '-']?.['@items'] && Object.entries(vm.statsDesc[tab + '-']['@items']).map(([name, stat]) => {
                  ...
                  return (
                    <div 
                      ...
                      <input
                        ...
                        onChange={(e) => vm.onStatChecked(statPath, e.target.checked)}
                        disabled={isDisabled} />
                      <OverlayTrigger
                        placement="auto-end"
                        delay={{ show: 500, hide: 0 }}
                        ...
                        <label
                          htmlFor={`stat-${statPath}-checkbox`}>
                            {stat.title} (per item)
                          </label>
                      </OverlayTrigger>
                    </div>
                ...
                {/* Non-KV Stats */}
                {vm.statsDesc[tab] && tab !== '@kv' && Object.entries(vm.statsDesc[tab]).map(([name, stat]) => {
          ...
          <div className="panel-footer spaced scroll-shadow">
            ...
            <div className="row row-min">
              <a
                ...
                onClick={vm.props.onDismiss}>
                Cancel
              </a>
              <button
                type="submit"
                disabled={!vm.getSelectedStatsLength()}>
                Save Chart
              </button>
            </div>
          </div>
