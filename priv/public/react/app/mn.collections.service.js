  ...
  constructor(http, mnBucketsService, mnPermissions) {
    ...
    this.stream.collectionBuckets = mnBucketsService.stream.bucketsMembaseEphemeral
      .pipe(map(buckets => buckets
                .filter(bucket => {
                  let scope = mnPermissions.export.getValue().cluster.collection[bucket.name + ':.:.'];
...
const MnCollectionsService = new MnCollectionsServiceClass(HttpClient, MnBucketsService, mnPermissions);
