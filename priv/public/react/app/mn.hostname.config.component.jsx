...
class MnHostnameConfigComponent extends MnLifeCycleHooksToStream {
  ...
  render() {
    ...
    const { postNodeInitHttpError, postClusterInitHttpError, isEnterprise } = this.state;
    ...
    const ipFamilyTip = (
      <Tooltip>
        Selecting IPv4 or IPv6 will instruct services in the cluster to listen on those addresses, though as a convenience some services will also listen on the other address family. Selecting 'IPv4/IPv6-only' will instruct services to ONLY listen on the selected addresses.
      </Tooltip>
    );
          ...
          <div>
            <div className="formrow">
              <div className="row">
                ...
                <small className="text-smaller">Fully-qualified domain name</small>
              </div>
              <FieldControl name="hostname" render={({ handler }) => (
                <input
                  type="text"
                  autoCorrect="off"
                  spellCheck="false"
            ...
            {isHostCfgEnabled && isEnterprise && (
              <FieldGroup name="hostConfig" render={() => (
                <div>
                  <div className="formrow">
                    <FieldControl name="nodeEncryption" render={({ handler }) => (
                      <>
                        <input
                          type="checkbox"
                          ...
                          {...handler()}
                        />
                        <label htmlFor="for-node-encryption" className="margin-right-quarter">
                          enable node-to-node encryption
                        </label>
                      </>
                    )} />
                    <OverlayTrigger placement="right" overlay={nodeEncryptionTip}>
                    ...
                    <div
                      className="error error-form"
                      hidden={!postClusterInitHttpError?.errors?.nodeEncryption}>
                      {postClusterInitHttpError?.errors?.nodeEncryption}
                    </div>
                  </div>

                  <label className="inline margin-right-quarter">IP Family Preference</label>
                  ...
                  <div className="formrow">
                    <div className="form-inline">
                      <FieldControl name="addressFamilyUI" strict={false} render={({ handler }) => {
                        ...
                        return (
                          ...
                          <input
                            ...
                            {...handlerSwitch}
                          />
                          <label htmlFor="for-use-ipv4-only">IPv4-only</label>
                          ...
                          <input
                            ...
                            {...handlerSwitch}
                          />
                          <label htmlFor="for-use-ipv6-only">IPv6-only</label>
                        </>
                      )}} />
                    </div>
                    {this.getAddressFamilyUI()?.hasError('ipvOnly') && (
                      <div className="error error-form">
                        Can't set IPv{this.getAddressFamilyUI().errors?.ipvOnly.value}-only from an IPv{this.getAddressFamilyUI().errors?.ipvOnly.kind} address; please access this server via an IPv{this.getAddressFamilyUI().errors?.ipvOnly.value} address
                      </div>
                    )}
                    {this.getAddressFamilyUI()?.warnings?.ipvOnly && (
                      <div className="error error-form">
                        You have selected IPv{this.getAddressFamilyUI().warnings?.ipvOnly.value}-only. If the domain name with which you have accessed this server is only available under IPv{this.getAddressFamilyUI().warnings?.ipvOnly.value} you will not be able to complete initialization
                      </div>
                    )}
                    <div
                      className="error error-form"
                    ...
                    <div
                      className="error error-form"
                      hidden={!postClusterInitHttpError?.errors?.afamilyOnly}>
