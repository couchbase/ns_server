...
class MnKeyspaceSelectorServiceClass {
  ...
  createCollectionSelector(options) {
    ...
    var filters = options.steps.reduce((acc, step) => {
      acc[step] = this.mnHelperService.createFilter(
        options.component,
        filterKey
      );
      return acc;
    ...
    var selectionDone = result.pipe(
      filter(
        (v) =>
          Object.values(v).filter((v) => !!v).length == options.steps.length
      )
    );
    ...
    var setStepsValuesToFields = () => {
      ...
      Object.keys(value).forEach((key) => {
        if (value[key]) {
          filters[key].group
            .get('value')
            .setValue(value[key] ? value[key][filterKey] : '');
        }
      });
    };

    var mapFocusToStep = merge.apply(
      merge,
    ...
    var step = merge(mapFocusToStep, selectionDone.pipe(mapTo('ok'))).pipe(
      shareReplay({ refCount: true, bufferSize: 1 })
    );

    var outsideClick = step.pipe(
      switchMap((v) => (v != 'ok' ? fromEvent(document, 'click') : NEVER))
    );
    ...
    var showHideDropdown = merge(outsideClick.pipe(mapTo('ok')), step).pipe(
      map((v) => v !== 'ok')
    );
    ...
    function getStepList([step, g]) {
      switch (step) {
        case 'bucket':
          return (
            options.buckets ||
            this.mnCollectionsService.stream.collectionBuckets
          );
        case 'scope':
          return g.bucket
            ? this.mnCollectionsService.getManifest(g.bucket.name).pipe(
                pluck('scopes'),
                catchError(() => of([]))
              )
            : of([]);
...
const MnKeyspaceSelectorService = new MnKeyspaceSelectorServiceClass(
  MnHelperService,
