...
class MnWizardJoinClusterComponent extends MnLifeCycleHooksToStream {
  ...
  render() {
    const {joinClusterHttpError, certificate, toggleConfigurationSection} = this.state;
    ...
    return (
      <div className="panel dialog-med dialog dialog-wizard">
        ...
        <FieldGroup control={this.joinClusterForm} strict={false} render={({submitted}) => (
          <form
            className="forms"
            onSubmit={(e) => {
              e.preventDefault();
            ...
            <div className="panel-content">
              <div 
                ...
                {joinClusterHttpError && joinClusterHttpError.map((error, i) => 
                  <div key={i}>{error}</div>
                )}
              </div>

              <div className="error error-form">
                <FieldControl name="clusterAdmin.hostname" render={({touched, errors}) => (
                  <div hidden={!(touched || submitted) || !errors?.required}>
                    Cluster name is required
                  </div>
                )} />

                <FieldControl name="clusterAdmin.user" render={({touched, errors}) => (
                  <div hidden={!(touched || submitted) || !errors?.required}>
                    Username is required
                  </div>
                )} />

                <FieldControl name="clusterAdmin.password" render={({touched, errors}) => (
                  <div hidden={!(touched || submitted) || !errors?.required}>
              ...
              <div>
                <div className="formrow">
                  <label htmlFor="for-hostname-field">Cluster Host Name/IP Address</label>
                  <FieldControl name="clusterAdmin.hostname" render={({handler}) => (
                    <input
                ...
                <div className="formrow">
                  ...
                  <FieldControl name="clusterAdmin.user" render={({handler}) => (
                    <input
                      type="text"
                      id="secure-user"
                      autoCorrect="off"
                ...
                <div className="formrow">
                  <div className="column">
                    <label htmlFor="secure-password">Cluster Admin Password</label>
                    <FieldControl name="clusterAdmin.password" render={({handler}) => (
                      <input
                        type="password"
                        id="secure-password"
                        autoCorrect="off"
              ...
              <div
                ...
                onClick={() => this.setState({toggleConfigurationSection: !this.state.toggleConfigurationSection})}>
            ...
            <div className="panel-footer">
              ...
              <button
                disabled={this.joinClusterForm.invalid}
                type="submit">
                Join Cluster
              </button>
            </div>
