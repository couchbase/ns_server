...
UIRouter.transitionService.onBefore({
  ...
  return mnPools.get().then(pools => {
    if (!pools.isInitialized) {
      return stateService.target('app.wizard.welcome', null, {location: false});
    } else {
      return true;
    }
  }, function (resp) {
    switch (resp.status) {
    case 401:
      return stateService.target('app.auth', locationService.search(), {location: false});
    }
  });
});
...
UIRouter.transitionService.onBefore({
  ...
  return mnPools.get().then(function (pools) {
    return pools.isInitialized ? true : trans.router.stateService.target("app.wizard.welcome");
  }, function (resp) {
    switch (resp.status) {
